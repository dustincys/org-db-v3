...
- *Linked File Indexing* :: Automatically indexes linked documents (PDF, DOCX, PPTX, etc.) for comprehensive search
...
org-db v3 automatically indexes linked documents (PDF, DOCX, PPTX, etc.) found in your org files. When you index an org file containing links like =[[./report.pdf]]= or =[[./proposal.docx]]=, the system:

1. Detects the linked file
2. Converts it to markdown using lightweight libraries:
   - PDFs: pymupdf4llm (~10MB memory)
   - DOCX: python-docx (text extraction)
   - PPTX: python-pptx (text extraction)
3. Chunks the content and generates embeddings
4. Makes it searchable alongside your org content
...
This shows all indexed linked files with:
- File type (PDF, DOCX, etc.)
- Source org file location
...
#+BEGIN_SRC emacs-lisp
;; Enable/disable linked file indexing (enabled by default)
(setq org-db-v3-enable-linked-files t)

;; Maximum file size for linked files (50MB default)
(setq org-db-v3-max-linked-file-size 52428800)
#+END_SRC

*** Search Results

When searching (semantic or fulltext), results from linked files are clearly marked:

#+BEGIN_EXAMPLE
File: /path/to/notes.org [linked PDF file: report.pdf]
Line: 42
Relevance: 0.834
Content:
...
(setq org-db-v3-enable-linked-files t)  ; enabled by default
(setq org-db-v3-max-linked-file-size 52428800)  ; 50MB default
...
- [X] Linked file indexing (PDF, DOCX, PPTX, etc.) with lightweight libraries
